...
ConfigParser::ConfigParser(const std::string& _filename, const std::string& logFile, Logger::Level logLevel)
    : filename(_filename), serverCount(0)
{
...
{
    LOG_INFO("ConfigParser instance destroyed");
}
...
void ConfigParser::parseLocationBlock(std::ifstream& configFile, ServerConfig& serverConfig) {
    ...
    // Aucune vérification pour '<' n'est nécessaire ici car "location <" est déjà le début du bloc
    // La première ligne pertinente après "location <" est directement une configuration de location

    while (std::getline(configFile, line)) {
        trim(line);

        if (line.empty() || line[0] == '#') {
            // Log pour les lignes vides ou les commentaires
        ...
        // Débute un nouveau bloc de configuration pour une location
        if (line[0] == '/' || line[0] == '~') {
            LOG_INFO("parseLocationBlock -> Traitement d'une nouvelle règle de location: " + line);
            LocationConfig locationConfig;
            std::istringstream lineStream(line);
            std::string path;
            getline(lineStream, path, ' '); // Extraire le chemin ou l'expression régulière
            locationConfig.path = path;

            // Marque une expression régulière
            if (path[0] == '~') {
            ...
            // Lecture du reste de la ligne pour la configuration
            parseLocationLine(lineStream, locationConfig);

            // Ajoute la configuration de location au serveur
...
void ConfigParser::parseLocationLine(std::istringstream& lineStream, LocationConfig& locationConfig) {
    ...
    while (lineStream >> config) {
        if (config == "autoindex") {
            ...
            // root et cgi_path sont spécifiés différemment, nécessitent une logique de parsing plus complexe
            if (config == "cgi") {
                lineStream >> locationConfig.cgi_path; // Assumer que le chemin CGI suit directement
                LOG_INFO("parseLocationBlock -> Chemin CGI spécifié: " + locationConfig.cgi_path);
            } else {
                // Le premier élément non-méthode est le root
                locationConfig.root = config;
                LOG_INFO("parseLocationBlock -> Racine définie pour la location: " + locationConfig.root);
                break; // Quitte la boucle, car le reste de la configuration nécessite un traitement différent
            }
        }
    }
}
...
void ConfigParser::resetISS(std::istringstream& _iss, const std::string& newStr)
{
    _iss.clear();
    _iss.str(newStr);
}
