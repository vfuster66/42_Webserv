...
HttpResponse RequestHandler::handleRequest(const HttpRequest& request)
{
    ...
    if (isMethodDenied(request.method, serverConfig)) {
        HttpResponse response;
        response.httpVersion = "HTTP/1.1";
...
HttpResponse RequestHandler::handleGetRequest(const HttpRequest& request)
{
    ...
    int port = extractPortFromHostHeader(hostHeader);
...
HttpResponse RequestHandler::handleCgiRequest(const HttpRequest& request)
{
    ...
    try
    {
        ...
        CgiHandler cgiHandler(scriptPath, request, *this);
...
int RequestHandler::extractPortFromHostHeader(const std::string& hostHeader)
{
    ...
    LOG_INFO("Default port 3000 used");
    return 3000;
}
...
bool RequestHandler::isMethodDenied(const std::string& method, const ServerConfig& serverConfig)
{
    for (size_t i = 0; i < serverConfig.denied_methods.size(); i++)
    {
        if (serverConfig.denied_methods[i] == method)
            return true;
    }
    return false;
}
