...
void ConfigParser::parseKeyValue(const std::string& line, ServerConfig& serverConfig)
{
    ...
    {
        LOG_WARNING("parseKeyValue -> La ligne ne se termine pas par ';': " + line);
        return;
    }
    std::istringstream _iss(line.substr(0, line.size() - 1)); // Enlever le ';'
    std::string key, rest;
    getline(_iss, key, ':');
    getline(_iss, rest);
    trim(key);
    ...
    if (key.empty())
    {
        LOG_WARNING("parseKeyValue -> Clé vide après traitement: " + line);
        return;
    }
    ...
    {
        char* end;
        long port = std::strtol(rest.c_str(), &end, 10); // 10 pour la base décimale

        // Vérifier si la conversion a réussi
        if (*end == '\0' && end != rest.c_str())
        { // Si end pointe sur le caractère null et n'est pas égal au début de la chaîne, la conversion a réussi
        serverConfig.port = static_cast<int>(port);
        LOG_INFO("parseKeyValue -> Port défini: " + rest);
        }
        else
        {
        LOG_WARNING("parseKeyValue -> Conversion de port échouée pour la valeur: " + rest);
        }
    }
