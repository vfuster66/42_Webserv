...
Server::Server(const std::string& configFilePath, const std::string& logFilePath, Logger::Level logLevel) : config(configFilePath, logFilePath, logLevel), requestHandler(NULL)
{
...
void Server::shutdownServer(const std::string& reason) {
    ...
    server_fds.clear();
    // Ajouter ici toute autre logique de nettoyage
    exit(EXIT_FAILURE); // Ou une méthode plus douce pour arrêter le serveur
}
...
void Server::setupServerSockets()
{
    ...
    for (size_t i = 0; i < servers.size(); ++i)
    {
        ...
        {
            perror("setsockopt");
            exit(1); // Ou gestion plus douce de l'erreur
        }
...
void Server::safeWrite(int fd, const std::string& data) {
    ...
    if (bytes_written < 0) {
        // Gérer l'erreur de write
        LOG_ERROR("Échec de l'envoi des données au client.");
        close(fd); // Fermer le socket en cas d'erreur
    }
}
        ...
        for (int i = 0; i <= max_fd; i++)
        {
            if (FD_ISSET(i, &read_fds))
            {
                ...
                else
                {
                    ...
                    {
                        std::string requestText(buffer, bytes_read);
                        HttpRequest httpRequest = HttpUtils_parseRequest(requestText);
                        HttpResponse httpResponse = requestHandler->handleRequest(httpRequest);

                        std::string connectionHeader = StaticRequestHandler::getHeader(httpRequest, "Connection");
