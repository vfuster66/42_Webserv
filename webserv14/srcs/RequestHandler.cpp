...
RequestHandler::RequestHandler() : current_port(-1) {
    // Add log for object creation
    LOG_INFO("Constructeur -> RequestHandler object created");
}

void RequestHandler::setServerConfigs(const std::vector<ServerConfig>& configs) {
    serverConfigs = configs;
    // Add log for setting server configurations
    LOG_INFO("setServerConfigs -> Server configurations set for RequestHandler");
}
...
HttpResponse RequestHandler::handleRequest(const HttpRequest& request) {
    // Log the start of handling request
...
std::string RequestHandler::getScriptPathFromUri(const std::string& uri) {
    ...
    if (uri.find(cgiBasePath) != std::string::npos) {
        // Log full CGI script path
        LOG_INFO("getScriptPathFromUri -> Full CGI script path: " + uri);
        return uri; // Return the URI directly without adding cgiBasePath in front
    } else {
        // Log error for unexpected URI path
        LOG_ERROR("getScriptPathFromUri -> Error: URI path does not contain the expected base path for CGI scripts.");
    }
    return ""; // Return an empty path or an error if the path does not start with the expected base path
}
