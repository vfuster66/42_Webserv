...
void ConfigParser::parseServerBlock(std::ifstream& configFile, ServerConfig& serverConfig)
{
    ...
    LOG_INFO("parseServerBlock -> Fin de l'analyse du bloc 'server'.");
}
...
void ConfigParser::parseKeyValue(const std::string& line, ServerConfig& serverConfig)
{
    ...
    else if (key == "denied_ips")
    {
        ...
        else if (key == "cgi_ext")
        {
            ...
            {
                trim(ext);
                serverConfig.cgi_ext.push_back(ext); // Stockez chaque extension validée
            }
        } else {
            // Pour chaque clé après "cgi_ext", vérifiez si elle correspond à une extension CGI
            ...
            {
                // Si la clé est une extension reconnue pour CGI
