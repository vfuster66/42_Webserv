...
std::string RequestHandler::getScriptPathFromUri(const std::string& uri) {
    std::cerr << "getScriptPathFromUri -> Débogage: URI reçu: " << uri << std::endl;
    std::string cgiBasePath = "/home/vfuster-/webserv4/www/cgi-bin/";
    if (uri.find(cgiBasePath) != std::string::npos) {
        std::cerr << "getScriptPathFromUri -> Chemin complet du script CGI: " << uri << std::endl;
        return uri; // Retourne directement l'URI sans ajouter cgiBasePath devant
    } else {
        std::cerr << "getScriptPathFromUri -> Erreur: Le chemin de l'URI ne contient pas le chemin de base attendu pour les scripts CGI." << std::endl;
    }
    return ""; // Retourner un chemin vide ou une erreur si le chemin ne commence pas par le chemin de base attendu
}

HttpResponse RequestHandler::handleCgiRequest(const HttpRequest& request) {
    std::cerr << "handleCgiRequest -> Début de handleCgiRequest avec URI: " << request.uri << std::endl;

    std::string scriptPath = getScriptPathFromUri(request.uri);
    std::cerr << "handleCgiRequest -> Chemin du script CGI obtenu: " << scriptPath << std::endl;

    CgiHandler cgiHandler(scriptPath, request);
    std::cerr << "handleCgiRequest -> CgiHandler construit avec scriptPath: " << scriptPath << std::endl;

    HttpResponse response = cgiHandler.executeScript();
    std::cerr << "handleCgiRequest -> Script CGI exécuté, préparation de la réponse HTTP" << std::endl;

    return response;
}

bool RequestHandler::isCgiRequest(const HttpRequest& request) {
    std::cerr << "isCgiRequest -> Débogage: isCgiRequest appelé avec URI: " << request.uri << std::endl;
    // Exemple de vérification basée sur l'extension du fichier
    std::string uri = request.uri;
    size_t lastDotPos = uri.find_last_of(".");
    if (lastDotPos != std::string::npos) {
        std::string extension = uri.substr(lastDotPos);
        std::cerr << "isCgiRequest -> Débogage: Extension extraite: " << extension << std::endl;
        if (extension == ".cgi" || extension == ".pl" || extension == ".php" || extension == ".py") {
            std::cerr << "isCgiRequest -> Débogage: Requête identifiée comme CGI." << std::endl;
            return true;
        }
    }
    std::cerr << "isCgiRequest -> Débogage: Requête non identifiée comme CGI." << std::endl;
    return false;
}
